...
import com.darren1112.dptms.sdk.starter.redis.masterSalve.MasterSlaveHolder;
...
public class MasterSlaveRedisUtil implements RedisUtil {
    ...
    @Override
    public void init() {
        LOGGER.info("MasterSlaveRedisUtil init = {}", this.ip);
        if (holder != null) {
            return;
        }
        holder = new MasterSlaveHolder();
        holder.setPrefix(this.prefix);
        holder.setMaxTotal(this.maxTotal);
        holder.setMaxIdle(this.maxIdle);
    ...
    @Override
    public void close() {
        LOGGER.info("MasterSlaveRedisUtil close = {}", this.ip);
        if (holder != null) {
            holder.close();
            holder = null;
        }
    ...
    public MasterSlaveHolder getHolder() {
        return holder;
    }

    public void setHolder(MasterSlaveHolder holder) {
        this.holder = holder;
    }
}
